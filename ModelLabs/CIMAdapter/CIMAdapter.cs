	...
	public class CIMAdapter
	{
		...
		private bool DoTransformAndLoad(Assembly assembly, ConcreteModel concreteModel, SupportedProfiles extractType, out Delta nmsDelta, out string log)
		{
			...
			try
			{
				...
				switch (extractType)
				{
					case SupportedProfiles.IES2:
					{
						// transformation to DMS delta					
						TransformAndLoadReport report = IES1Importer.Instance.CreateNMSDelta(concreteModel);

						if (report.Success)
						{
							nmsDelta = IES1Importer.Instance.NMSDelta;
							success = true;
						}
						else
						{
